const Egg = {
    ...
    initializeTimer: (startTime, endTime, label) => {
        Egg.endTime = endTime;
    ...
    updateParts: function(Time) {
        ...
        var timeText = slabel + "<ul>" + yearText + monthText + dayText + hourText + minText + secText + "</ul>";
        Egg.updateTitle(clockTime.join(":") + (Egg.label && Egg.label !== "" ? " : " + Egg.label : "") + " left");
...
function getTimeText(time, label) {
    var timeText = "";
    if (time > 0) {
        timeText = `<li><span id="${label}">${time}</span>${label + getSModifier(time)}`;
